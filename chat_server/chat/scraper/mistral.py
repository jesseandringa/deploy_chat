...
def trim_text_using_ollama(text):
    response = ollama.generate(
        model="llama2",
        prompt=f"Trim down the the following text by getting rid of any nonsense chars and strings {text}",
    )
...
def check_if_text_is_helpful_using_ollama(text):
    response = ollama.generate(
        model="llama2",
        prompt=f"If the following text is not helpful for understanding laws/ regulations/ and information about a city or municipality respond only with the word false, else response only the word true. Here is the text:  {text}",
    )
